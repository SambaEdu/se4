...
then
	...
	if [ -e /etc/sambaedu/sambaedu.conf ]
	then
...
#	        dbpass=`cat /var/www/se3/includes/config.inc.php | grep "dbpass=" | cut -d = -f 2 |cut -d \" -f 2`
	...
	then
		...
		then
		  ...
		  then
			...
			for i in  sambaedu # $LISTE_BASE  => ocs non obligatoire!
			do
	...
	then
		...
		if [ -e /var/www/sambaedu/includes/dbconfig.inc.php ]
		then
			...
			dbhostinvent="localhost"
		        dbnameinvent="ocsweb"
		        dbuserinvent="ocs"
		        dbpassinvent=`cat /var/www/sambaedu/includes/dbconfig.inc.php | grep "PSWD_BASE" | cut -d\" -f4`
			# test la connexion avec ce compte
